...
# drop duplicate column using np.array_equal()
...
def normalize_features(train, test):
    ...
    for f in flist:
        # if max is too high, clip it to (2nd max + 1)
        if train[f].max() == 9999999999.0:
            fmax = train.loc[train[f]<9999999999.0, f].max()
            train.loc[train[f]==9999999999.0, f] = fmax + 1

        # if only one negative value, set it to -1
        # devide positive value by max : using max of test is not clean!
        ...
        # case where there is no negatives, only divide those above zero
        ...
        # case where more than one negative values, divide by max anyway
        if len(train.loc[train[f]<0, f].value_counts()) > 1:
            fmax = max(np.max(train[f]), np.max(test[f]))
            if fmax > 0:
